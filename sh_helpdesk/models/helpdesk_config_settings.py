...
class ResCompany(models.Model):
    ...
    sh_default_user_id_domain = fields.Char("default users domain", store=True)
...
class HelpdeskSettings(models.TransientModel):
    ...
    sh_default_user_id = fields.Many2one(
        ...
        string="Default Assign User",
        readonly=False,
        related='company_id.sh_default_user_id', store=True)
    ...
    sh_default_user_id_domain = fields.Char("default users domain", related='company_id.sh_default_user_id_domain',readonly=False, store=True)
   
    @api.onchange('sh_default_team_id')
    def onchange_sh_default_team_id(self):
        for rec in self:
            domain = [('id', '<', 0)]
            if rec.sh_default_team_id:
                domain = [('id', 'in', rec.sh_default_team_id.team_members.ids)]
                rec.sh_default_user_id_domain = json.dumps(domain)           
